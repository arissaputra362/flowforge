...
@push('scripts')
    <script>
        ...
        function addStep() {
            ...
            // Cek apakah step sebelumnya adalah condition
            const prevStep = index > 0 ? steps[index - 1] : null;
            const prevIsCondition = prevStep?.type === 'condition';

            // Branch step tidak boleh depends_on condition step
            const dependsOn = (index === 0 || prevIsCondition) ? [] : [steps[index - 1].id];
        ...
        <div class="bg-white/5 border border-white/10 rounded-lg p-4 space-y-3">
            ...
            <p class="text-xs text-slate-500 mt-1">
                Depends on: ${step.depends_on.length ? step.depends_on.join(', ') : 'none (root / branch target)'}
            </p>
                ...
                return `
                    <div>
                        ...
                        <input type="text"
                            ...
                            class="w-full bg-black/40 text-white p-2 rounded"
                            placeholder="step.step1.output.status == 200">
                    </div>
